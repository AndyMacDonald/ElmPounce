...
negamax scoreFn board budget alpha beta color =
    if budget <= 0 || Set.isEmpty board.moves then
        ...
        let
            candidates =
                List.map (\x -> ( x, (Board.update (Clicked x) board) )) (Set.toList board.moves)

            newBudget =
                budget // (Set.size board.moves) - 1
        in
...
alphaBetaPruner scoreFn boards budget alpha beta color best =
    case boards of
        ( idx, board ) :: rest ->
            let
                ( negScore, _ ) =
                    negamax scorer board budget -beta -alpha -color
